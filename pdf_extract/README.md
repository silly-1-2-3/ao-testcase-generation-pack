...
当前版本只支持纵式表格：第一行是表头，后续内容按列向下读取；这种形式已在示例流程中验证可用。当前版本不能识别以第一列作为表头、向右读取数据的横式表格，即使通过 `--headers` 提供了该表头也无法识别。若输入中存在横式表格，需要先转换版式，或扩展程序的列方向表头识别逻辑。

识别到的子表格会写入所属父单元格的 `nested_tables`。如果下一页重复相同表头且序号列留空，程序会用表头单元格的 x 坐标范围匹配列，把续页文本追加到上一页最后一行的对应列，并把续页中的子表挂入上一页内容单元格的 `nested_tables`；下一页出现新序号时则开始新行，不会错误拼接。
